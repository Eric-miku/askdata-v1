...
def _truncate_value(value: Any) -> Any:
    if isinstance(value, str):
        return value[:MAX_SNAPSHOT_LENGTH] + "..." if len(value) > MAX_SNAPSHOT_LENGTH else value
    return value
...
def traced(func: Callable) -> Callable:
    ...
    def wrapper(state: Any, *args, **kwargs) -> Dict[str, Any]:
        ...
        input_snapshot = state.model_dump() if hasattr(state, "model_dump") else dict(state)
