...
class WorkflowState(BaseModel):
    model_config = ConfigDict(
        arbitrary_types_allowed=True, validate_default=True, extra="ignore"
    )
    ...
    validation_errors: List[str] = Field(
        default_factory=list, description="SQL语法/风险校验错误"
    )
    execution_result: Dict = Field(
        default_factory=dict, description="SQL执行结果 {columns:[], rows:[]}"
    )
    ...
    trace: Annotated[List[Dict], operator.add] = Field(
        default_factory=list, description="节点执行追踪日志"
    )
