...
def generate_sql_node(
    state: WorkflowState, model_client: ModelClient
) -> Dict[str, Any]:
    if not model_client:
...
def validate_sql_node(
    state: WorkflowState, db_dialect: str = "mysql"
) -> Dict[str, Any]:
    errors = []
...
def analyze_result_node(
    state: WorkflowState, result_analyzer: ResultAnalyzer
) -> Dict[str, Any]:
