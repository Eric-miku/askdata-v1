...
class BirdSchemaIndex:
    ...
    def Build(self, databases: list[Any], instructions_dir=None):
        self.databases = {_Get(database, "databaseId", "database_id"): database for database in databases}
        if instructions_dir:
    ...
    def Retrieve(self, database_id: str, question: str) -> dict[str, Any]:
        ...
        matched_tables = []
        matched_columns = []
        tables = list(_Get(database, "tables", default=[]))

        for table in tables:
            table_name = _Get(table, "tableName", "table_name")
            table_matched = bool(question_tokens & _Tokens(table_name))
            column_matches = []
            for column in _Get(table, "columns", default=[]):
                column_name = _Get(column, "columnName", "column_name")
        ...
        if not matched_tables:
            matched_tables = [
                {"table_name": _Get(table, "tableName", "table_name"), "reason": "Included for compact database context."}
        ...
        for table in tables:
            ...
            if table_name not in selected_names:
                continue
            for column in _Get(table, "columns", default=[]):
                if _Get(column, "isPrimary", "is_primary", default=False):
                    exists = any(
                        item["table_name"] == table_name and item["column_name"] == _Get(column, "columnName", "column_name")
        ...
        matched_joins = []
        for key in _Get(database, "foreignKeys", "foreign_keys", default=[]):
            left_table = _Get(key, "leftTable", "left_table")
            right_table = _Get(key, "rightTable", "right_table")
            if left_table in selected_names or right_table in selected_names:
                matched_joins.append({
                    "left_table": left_table,
                    "left_column": _Get(key, "leftColumn", "left_column"),
                    "right_table": right_table,
                    "right_column": _Get(key, "rightColumn", "right_column"),
                })

        schema_prompt = self.BuildSchemaPrompt(database, selected_names, matched_joins)
        return {
            "database_id": database_id,
            "database_path": _Get(database, "databasePath", "database_path", default=""),
    ...
    def _ColumnDict(self, table_name: str, column: Any, reason: str) -> dict[str, str]:
        return {
            ...
            "column_name": _Get(column, "columnName", "column_name"),
            "column_type": _Get(column, "columnType", "column_type", default="text"),
            "reason": reason,
        }

    def BuildSchemaPrompt(self, database: Any, selected_names: set[str], joins: list[dict[str, str]]) -> str:
        database_id = _Get(database, "databaseId", "database_id")
        database_path = _Get(database, "databasePath", "database_path", default="")
        ...
        if instructions:
            lines.append(f"\n--- Business Context ---\n{instructions}\n---")
        tables = _Get(database, "tables", default=[])
        for table in tables:
            table_name = _Get(table, "tableName", "table_name")
            if selected_names and table_name not in selected_names and len(tables) > 8:
                continue
            columns = ", ".join(
                f"{_Get(column, 'columnName', 'column_name')} {_Get(column, 'columnType', 'column_type', default='text')}".strip()
                for column in _Get(table, "columns", default=[])
            )
